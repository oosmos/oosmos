...
#
def Copy(From: str, FromExtensionsArg: str, To: str, ToExtensionsArg: str, FilesArg: str, bCopyOosmos: bool = True):
	FromExtensions = FromExtensionsArg.split()
	ToExtensions   = ToExtensionsArg.split()

	if bCopyOosmos:
		for Index, FromExtension in enumerate(FromExtensions):
			ToExtension = ToExtensions[Index]

			FromFile = f'./Source/oosmos.{FromExtension}'
			ToFile   = f'./{To}/oosmos.{ToExtension}'
	...
	for File in Files:
		for Index, FromExtension in enumerate(FromExtensions):
			...
			FromFile = f'./{From}/{File}.{FromExtension}'
			ToFile   = f'./{To}/{File}.{ToExtension}'
			oosmos.CopyFileReadOnly(FromFile, ToFile)
